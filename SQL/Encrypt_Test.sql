...
DECLARE
    ...
    encryption_type PLS_INTEGER := SYS.DBMS_CRYPTO.ENCRYPT_AES128 + SYS.DBMS_CRYPTO.CHAIN_CBC + SYS.DBMS_CRYPTO.PAD_PKCS5;
BEGIN
...
create or replace view  NV_DECRYPT_LUONG_PHUCAP as
select MANV,TENNV,PHAI,NGAYSINH,DIACHI,SODT,CAST (DECRYPT_LUONG_PHUCAP(LUONG) AS varchar2 (255)) LUONG,
CAST (DECRYPT_LUONG_PHUCAP(PHUCAP) AS varchar2 (255)) PHUCAP,VAITRO,MANQL,PHG
from NHANVIEN WHERE MANV = SYS_CONTEXT('USERENV', 'SESSION_USER');


GRANT SELECT ON NV_DECRYPT_LUONG_PHUCAP TO ROLE_NHANVIEN;
--SELECT * FROM P_ENCRYPT.NV_DECRYPT_LUONG_PHUCAP;
...
GRANT SELECT ON view_decrypt_LUONG_PHUCAP TO ROLE_TAICHINH;
GRANT SELECT ON view_decrypt_LUONG_PHUCAP TO ROLE_BGD;
